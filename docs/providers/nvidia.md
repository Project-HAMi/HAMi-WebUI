...
- **MIG**: the scheduler reserved the instance in `hami.io/vgpu-mig-allocations`
  for that container slot and GPU, and the GPU is registered in MIG mode or its
  node's registration cannot be read. The reservation names the profile and its
  placement. Allocations recorded before HAMi v2.10 carry the template and slot
  in the device UUID (`GPU-…[1-2]`); they show as MIG without a profile.
- **HAMi-core**: the GPU is registered in HAMi-core mode and holds no MIG
  reservation, or its node's registration cannot be read and the Pod asked for
  HAMi-core or mps in `nvidia.com/vgpu-mode`.
- **Unknown**: anything else, such as a MIG GPU without its reservation, a
  reservation for another GPU, a GPU missing from its node's registration, an
  unreadable registration with nothing else naming the mode, or a reservation
  annotation that HAMi's own decoder would reject. Memory and compute still
  come from the allocation.
...
none, and the summary lists how many instances of each profile would still fit
if only that profile were placed. When an allocation on the GPU has no
placement, such as one recorded before HAMi v2.10 or one whose record is
unusable, the GPU is drawn by memory instead. A HAMi-core GPU shows its memory
and compute quotas, since nothing is carved out.
