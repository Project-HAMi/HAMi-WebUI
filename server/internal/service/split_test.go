...
func TestUnknownShapesSayWhy(t *testing.T) {
	containers := []*biz.Container{
		...
		{Name: "two-reasons", PodUID: "pod-4", Status: biz.ContainerStatusSuccess, ContainerDevices: biz.ContainerDevices{
			{UUID: "GPU-M", Type: "NVIDIA", Vendor: biz.NvidiaGPUDevice, Shape: biz.SplitShapeUnknown, ShapeReason: "mig_reservation_missing"},
			{UUID: "GPU-N", Type: "NVIDIA", Vendor: biz.NvidiaGPUDevice, Shape: biz.SplitShapeUnknown, ShapeReason: "device_mode_unknown"},
		}},
		{Name: "ambiguous", PodUID: "pod-3", Status: biz.ContainerStatusSuccess, ContainerDevices: biz.ContainerDevices{
			{UUID: "NPU-A", Type: "Ascend910B3", Vendor: biz.AscendGPUDevice, Shape: biz.SplitShapeUnknown, CoreAllocationUnknown: true, CoreReason: "mode_ambiguous"},
		}},
	}
	...
	// One reason cannot speak for devices that give different ones.
	if item := byName["two-reasons"]; item.AllocationShape != biz.SplitShapeUnknown || item.AllocationShapeReason != "" ||
		item.Devices[1].AllocationShapeReason != "device_mode_unknown" {
		t.Fatalf("two-reasons = %+v", item)
	}
	if device := byName["ambiguous"].Devices[0]; device.AllocatedCoresReason != "mode_ambiguous" || device.GetAllocatedCoresKnown() {
		t.Fatalf("ambiguous device = %+v", device)
	}
	if device := byName["broken"].Devices[0]; device.AllocatedCoresReason != "" {
		t.Fatalf("a known share carries a reason: %+v", device)
	}
}
