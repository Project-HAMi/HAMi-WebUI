...
// The configuration only matters where Ascend devices are registered; every
// model HAMi ships uses the Ascend prefix, as the server's fallback does.
export const affectsAscendDevices = (deviceTypes = []) => deviceTypes.some(
  (type) => typeof type === 'string' && type.startsWith('Ascend'),
);

// Warn once the configuration is decided and unreadable; loading says nothing yet.
export const shouldWarnAboutDeviceConfig = (config, deviceTypes = []) => Boolean(
  config && config.state && config.state !== 'loaded' && config.state !== 'loading' && affectsAscendDevices(deviceTypes),
);
