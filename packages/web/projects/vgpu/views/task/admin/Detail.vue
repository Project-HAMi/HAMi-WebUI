...
watch([() => detail.value?.podUid, distinctDeviceIds], ([podUid, uuids]) => {
...
const relatedGpuCountText = computed(() => {
  const cards = distinctDeviceIds.value.length;
  const instances = safeDeviceIds.value.length;
  return instances > cards
    ? t('task.relatedGpuInstances', { count: cards, instances })
    : t('task.relatedGpuCards', { count: cards });
});
