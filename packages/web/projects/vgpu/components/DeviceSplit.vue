  ...
  <section class="device-split" :aria-busy="status === 'loading'">
    <header class="device-split__head" :class="{ 'has-device': showDevice }">
      <div v-if="showDevice" class="device-split__device">
        ...
        <div class="device-split__identity">
          <span class="device-split__model">
            {{ device.type || '--' }}
            <t-tag v-if="modeText" size="small" variant="light">{{ modeText }}</t-tag>
          </span>
      ...
      <p v-if="status === 'ready'" class="device-split__summary">
        <span v-for="(part, i) in summary" :key="i" :class="{ 'is-warning': part.warning }">{{ part.text }}</span>
        <span v-if="hasStranded" class="device-split__legend">
    ...
    <template v-else>
      <!-- MIG: instances in the placement space the plugin registered. -->
      <div v-if="split.kind === 'mig' && split.slots" class="split-mig">
        <div
          class="split-mig__grid"
          role="group"
          :aria-label="summaryText"
          :style="{ '--slots': split.slots, '--lanes': split.lanes }"
        >
          <span
            ...
            aria-hidden="true"
          />
          <t-tooltip v-for="block in split.blocks" :key="block.key" placement="top">
            <template #content>
              <span class="split-tip__title">{{ tipTitle(block) }}</span>
              <span class="split-tip__line">{{ workloadText(block.workload) }}</span>
            </template>
            <button
              type="button"
              class="split-part"
        ...
        <div class="split-mig__ruler" :style="{ '--slots': split.slots }" aria-hidden="true">
          <span v-for="slot in split.slots" :key="slot">{{ slot - 1 }}</span>
        </div>
      </div>

      <!-- HAMi-core: quotas on one shared device. -->
      <div v-else-if="split.kind === 'shared'" class="split-meters">
        <div v-for="row in meters" :key="row.key" class="split-meter">
          ...
          <div class="split-meter__track" role="img" :aria-label="`${row.name}: ${row.value}`">
            <span
      ...
      <!-- Templates and whole cards: fixed parts of the device memory. -->
      <div v-else class="split-strip" role="group" :aria-label="summaryText" :style="{ '--gaps': `${stripGaps}px` }">
        <t-tooltip v-for="block in split.blocks" :key="block.key" placement="top">
          <template #content>
            <span class="split-tip__title">{{ tipTitle(block) }}</span>
            <span class="split-tip__line">{{ workloadText(block.workload) }}</span>
      ...
      <ul v-if="split.holders.length" class="split-rows" :class="`is-${split.kind}`">
        ...
        >
          ...
          <span class="split-row__size">{{ sizeText(holder) }}</span>
...
const props = defineProps({
  ...
  // The accelerator page already names the device.
...
const skeletonRows = [
  ...
  { width: '100%', height: '32px', marginTop: '12px' },
];
...
const computeText = (holder) => {
  ...
  return holder.cores ? `${trimmed(holder.cores)}%` : t('card.split.unlimited');
};
const labelOf = (holder) => {
  if (holder.shape === 'mig') return holder.name || 'MIG';
  ...
  if (holder.shape === 'soft') return 'HAMi-core';
...
const sizeText = (holder) => [memoryText(holder.memoryMiB), `${t('card.split.compute')} ${computeText(holder)}`].join(' · ');
// MIG rows name the slices they sit in, in a column of their own.
...
const tipTitle = (holder) => [labelOf(holder), placementText(holder), sizeText(holder)].filter(Boolean).join(' · ');
...
const describe = (holder) => [
  tipTitle(holder),
  workloadText(holder.workload),
  holder.current ? t('card.split.thisWorkload') : '',
].filter(Boolean).join(' — ');
const locationOf = (holder) => buildWorkloadDetailLocation({ podUid: holder.workload.podUid, name: holder.workload.container });
const open = (holder) => {
  const location = !holder.current && locationOf(holder);
...
const meters = computed(() => {
  ...
  const memoryValue = memory.total
    ...
    : `${memoryText(memory.used)} · ${t('card.split.memoryUnknown')}`;
  ...
  return [
    ...
    { key: 'compute', name: t('card.split.compute'), meter: compute, value: computeValue.join(' · ') },
  ];
});
...
const summary = computed(() => {
  ...
  const parts = [];
  if (value.kind === 'mig') {
    if (value.slots) parts.push({ text: t('card.split.slicesUsed', { used: value.usedSlots, total: value.slots }) });
    if (value.fits) {
      parts.push({
        text: value.fits.length
          ? t('card.split.fits', { list: value.fits.map((fit) => `${fit.name} ×${fit.count}`).join('、') })
          : t('card.split.fitsNone'),
      });
    }
...
// Too narrow to name the part; the tooltip and the row below still do.
...
// One grid for both meters, so labels, tracks and values line up.
.split-meters {
  display: grid;
...
// One grid for every row, so the columns line up and stay close together.
.split-rows {
  display: grid;
  ...
  // MIG rows lead with the slices, so they read against the ruler above.
  &.is-mig {
    grid-template-columns: 8px max-content max-content max-content minmax(0, 1fr);
  }

  // HAMi-core rows have no part to name.
  &.is-shared {
...
.split-row {
  ...
  // The workload's own part, marked the way a selected row is.
  &.is-current {
    background: #f2f7ff;
...
@media (max-width: 720px) {
  ...
  // Each part reads as a stack under its swatch.
  .split-rows,
  .split-rows.is-mig,
