  ...
  for (const container of containers) {
    (container?.devices || []).forEach((allocated, position) => {
      ...
      holders.push({
        ...
        placement: size > 0 && start !== undefined ? { start, size } : undefined,
        workload,
...
const migProfiles = (device) => (device.migProfiles || [])
  .map((profile) => ({
    ...
    placements: (profile?.placements || [])
      .map((placement) => ({ start: index(placement?.start), size: positive(placement?.size) }))
      .filter((placement) => placement.start !== undefined && placement.size > 0),
  }))
...
export const buildMigSplit = (device, holders) => {
  ...
  return {
    ...
    holders: blocks,
    // Without registered profiles, nothing tells an open slot from a stranded one.
    cells: occupied.map((used, slot) => ({
      slot,
      state: used ? 'used' : (!profiles.length || usable[slot] ? 'open' : 'stranded'),
    })),
    usedSlots: occupied.filter(Boolean).length,
